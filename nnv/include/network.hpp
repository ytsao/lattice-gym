...
public:
  ...
  bool read_vnnlib(std::string vnnlib_filename) {
    ...
    if (!vnnlib_file.is_open()) {
      Logger::log(Logger::Level::ERROR,"Could not open file " + vnnlib_filename);
      return false;
    }
  ...
  void dump_bounds_at_layer(size_t layer_id) {
    if (layer_id >= layers.size()) {
      Logger::log(Logger::Level::ERROR,"Layer id " + std::to_string(layer_id) + " is out of range.");
      return;
    }

    for (size_t neuron_id = 0; neuron_id < layers[layer_id].layer_size; ++neuron_id) {
      Logger::log(Logger::Level::INFO,"Layer " + std::to_string(layer_id) + ", Neuron " + std::to_string(neuron_id) + " : [" + std::to_string(layers[layer_id].neurons[neuron_id].bounds.getLb()) + ", " + std::to_string(layers[layer_id].neurons[neuron_id].bounds.getUb()) + "]");
      Logger::log(Logger::Level::INFO,"Bias: [" + std::to_string(layers[layer_id].lower_biases[neuron_id]) + ", " + std::to_string(layers[layer_id].upper_biases[neuron_id]) + "]");
    }
