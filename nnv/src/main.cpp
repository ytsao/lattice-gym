...
void verify(const std::string &vnnlib_path, const std::string &onnx_path,
  ...
  if (!nnv.read_vnnlib(vnnlib_path)) {
    Logger::log(Logger::Level::ERROR,"Could not parse vnnlib file " + vnnlib_path);
    return;
  }
  ...
  if (std::strcmp(propagation_method.c_str(), "ibp") == 0) {
    ...
    if (ibp.execute(nnv)) {
      Logger::log(Logger::Level::INFO,"The naive interval propagation result is UNSAT");
      num_verified++;
    } else {
      Logger::log(Logger::Level::WARN,"The naive interval propagation result is not able to have a conclusive verification result.");
    }
  } else if (std::strcmp(propagation_method.c_str(), "sip") == 0) {
    SymbolicIntervalPropagation sip;
    if (sip.execute(nnv)) {
      Logger::log(Logger::Level::INFO,"The symbolic interval propagation result is UNSAT");
      num_verified++;
    } else {
      Logger::log(Logger::Level::WARN,"The symbolic interval propagation result is not able to have a conclusive verification result.");
    }
  } else if (std::strcmp(propagation_method.c_str(), "deeppoly") == 0) {
    DeepPolyPropagation deeppoly;
    if (deeppoly.execute(nnv)) {
      Logger::log(Logger::Level::INFO,"The deeppoly propagation result is UNSAT");
      num_verified++;
    } else {
      Logger::log(Logger::Level::WARN,"The deeppoly propagation result is not able to have a conclusive verification result.");
    }
  }
  // nnv.dump_all_bounds();
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2) {
    ...
    for (Utility::TaskEntry task : tasks) {
      verify(task.vnnlib_path, task.onnx_path, task.propagation_method,num_verified);
    }
  } else if (argc == 4) {
  ...
  Logger::log(Logger::Level::INFO,"Number of verified: " + std::to_string(num_verified));
